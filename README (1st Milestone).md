...
- Late data handling
- Windowed analytics
- Forming the foundation for a real-time analytics pipeline using Spark Structured Streaming *(Milestone 2)*
...
|-------------|------|--------------------------|
| Paul Khairallah | Tech Lead / Architect | Streaming architecture, feed separation, future Spark Structured Streaming integration |
...
│   └── order_events.avsc
├── docs                 # Detailed design notes and justification
└── README.md
```
...
- `event_sequence` (nullable int): intra-order sequence number for ordering guarantees
...
- `latitude` (nullable double): courier latitude — present on `LOCATION_UPDATE` events
- `longitude` (nullable double): courier longitude — present on `LOCATION_UPDATE` events
